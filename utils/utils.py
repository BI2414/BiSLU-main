...
def get_useful_embedding(embeddings, mask):
    ...
    Args:
        embeddings: Tensor of shape [batch_size, seq_len, seq_len, embed_dim]
        mask: Tensor of shape [batch_size, seq_len, seq_len] indicating valid positions

    Returns:
        Tensor of shape [num_valid_positions, embed_dim] containing only valid embeddings
    """
    # Reshape embeddings and mask to 2D
    batch_size, seq_len, _, embed_dim = embeddings.shape
    embeddings = embeddings.reshape(-1, embed_dim)  # [batch_size*seq_len*seq_len, embed_dim]
    mask = mask.reshape(-1)  # [batch_size*seq_len*seq_len]

    # Select only valid positions (where mask != 0)
    indices = mask.nonzero(as_tuple=False).squeeze(-1).long()
    valid_embeddings = embeddings.index_select(0, indices)
