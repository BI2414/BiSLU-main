...
class BiaffineLayer(nn.Module):
    def __init__(self, inSize1, inSize2, classSize, dropout=0.3):
        ...
        self.bilinearMap = nn.Parameter(
            torch.FloatTensor(inSize1 + 1, classSize, inSize2 + 1))
    ...
    def forward(self, x1, x2):
        ...
        x1 = torch.cat(
            (x1, torch.ones([batch_size, bucket_size, 1]).to(x1.device)), axis=2)
        x2 = torch.cat(
            (x2, torch.ones([batch_size, bucket_size, 1]).to(x2.device)), axis=2)
        # Static shape info
